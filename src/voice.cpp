...
Volume default_volume(VoiceId voice) { return Volume(100 - 20 * (voice % 4)); }
Octave default_octave(VoiceId voice) { return Octave(6 - voice % 4); }
...
uint32_t VoiceManager::get_bpm() const { return bpm; }
void VoiceManager::set_bpm(uint32_t _bpm) { bpm = _bpm; }
